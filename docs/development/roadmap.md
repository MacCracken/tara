...
- [ ] Add `#[inline]` to `sse::coeff` — hot-path Horner evaluation used in all ZAMS calculations
- [ ] Document hardcoded phase multipliers (1.1, 1.3, 1.4, 1.5, 1.6) in `evolutionary_phase` with forward references — will drift if `main_sequence_lifetime` changes
- [ ] Clarify `class_temp_range` return order — returns (max, min) which is inverted relative to naming convention
- [ ] Add convergence check to Newton iteration in `soorat.rs` (`temperature_to_bv`) — currently 20 fixed iterations with no convergence guard
